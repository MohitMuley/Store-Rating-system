...
const StoreDetails = () => {
  ...
  const handleRatingSubmit = (userRating, averageRating) => {
    ...
    // Update ratings list for current user
    setRatings((prev) => {
      const existingIndex = prev.findIndex((r) => r.user._id === user.id);
      if (existingIndex !== -1) {
        const updated = [...prev];
        updated[existingIndex].rating = userRating;
        return updated;
      } else {
  ...
  return (
    <div>
      ...
      <p className="mb-4">Overall Rating: {store.averageRating || 0}</p>
